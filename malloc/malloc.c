...
#if USE_TCACHE //!tcache机制相关 宏函数与参数
/* We want 64 entries.  This is an arbitrary limit, which tunables can reduce.  */
# define TCACHE_MAX_BINS		64 //!tcache bins表长度
...
# else
#  define DEFAULT_MMAP_THRESHOLD_MAX (4 * 1024 * 1024 * sizeof(long)) //!32MB
# endif
#endif
...
#define MIN_CHUNK_SIZE        (offsetof(struct malloc_chunk, fd_nextsize)) //! fd_nextsize在malloc_chunk结构体中的偏移
                                                                           //!  这里用来代表最小堆的大小。
/* The smallest size we can malloc is an aligned minimal chunk */

#define MINSIZE  \
  (unsigned long)(((MIN_CHUNK_SIZE+MALLOC_ALIGN_MASK) & ~MALLOC_ALIGN_MASK)) //!(0x20+0x0f)&~0x0f
...
#define misaligned_chunk(p) \
  ((uintptr_t)(MALLOC_ALIGNMENT == 2 * SIZE_SZ ? (p) : chunk2mem (p)) \ //! p & 0xf
   & MALLOC_ALIGN_MASK)
...
#define unlink(AV, P, BK, FD) {                                            \
    if (__builtin_expect (chunksize(P) != prev_size (next_chunk(P)), 0))      \//?长度不匹配时触发，但此时并没有发生size修改
      malloc_printerr (check_action, "corrupted size vs. prev_size", P, AV);  \
    FD = P->fd;								      \//?下一个堆的指针指向原p即释放目标
    BK = P->bk;								      \//?上一个堆的指针p的上一个的上一个
    if (__builtin_expect (FD->bk != P || BK->fd != P, 0))		      \//?未形成双链表判定
      malloc_printerr (check_action, "corrupted double-linked list", P, AV);  \
    else {								      \
        FD->bk = BK;						\//?将p的上一个堆改成p的上个的上个
        BK->fd = FD;						\//?将p的上个的上个的下个改成p，剥离p的上一个
        if (!in_smallbin_range (chunksize_nomask (P))			\//?是否属于smallbin大小
            && __builtin_expect (P->fd_nextsize != NULL, 0)) {		      \//?是否存在后继不同大小的chunk
	    if (__builtin_expect (P->fd_nextsize->bk_nextsize != P, 0)	      \//?是否构成双链
		|| __builtin_expect (P->bk_nextsize->fd_nextsize != P, 0))    \//?是否构成双链
	      malloc_printerr (check_action,				      \
			       "corrupted double-linked list (not small)",    \
			       P, AV);					      \
            if (FD->fd_nextsize == NULL) {				      \//?解第二组链
                if (P->fd_nextsize == P)				      \
...
{
  /* Serialize access.  */
  __libc_lock_define (, mutex);//!定义互斥锁？

  /* Flags (formerly in max_fast).  */
  int flags;//!存放各种状态标志,malloc.c-#1630排

  /* Fastbins */
  mfastbinptr fastbinsY[NFASTBINS];//!fast bin表

  /* Base of the topmost chunk -- not otherwise kept in a bin */
  mchunkptr top;//! top堆地址

  /* The remainder from the most recent split of a small request */
  mchunkptr last_remainder;//! 切割后余下chunk

  /* Normal bins packed as described above */
  mchunkptr bins[NBINS * 2 - 2];//! unsort,small,large bin表

  /* Bitmap of bins */
  unsigned int binmap[BINMAPSIZE];//!  bitmap算法的数据结构，用于加速对bins的搜索

  /* Linked list */
  struct malloc_state *next;//! 如果存在，指向下一个malloc_state;否则指向自己的首地址。

  /* Linked list for free arenas.  Access to this field is serialized
     by free_list_lock in arena.c.  */
  struct malloc_state *next_free;//!下一个释放的malloc_state?

  /* Number of threads attached to this arena.  0 if the arena is on
     the free list.  Access to this field is serialized by
     free_list_lock in arena.c.  */
  INTERNAL_SIZE_T attached_threads;//!此arena中的线程数

  /* Memory allocated from the system in this arena.  */
  INTERNAL_SIZE_T system_mem;//!系统为此arena分配的内存
  INTERNAL_SIZE_T max_system_mem;//!系统为此arena会分配的最大内存
};

struct malloc_par
{
  /* Tunable parameters */
  unsigned long trim_threshold; //!top堆收缩阈值， 可通过mallocpt()设置，一般mmap分配阈值的两倍
  INTERNAL_SIZE_T top_pad;      //!表示在分配内存时，额外多分配的内存
  INTERNAL_SIZE_T mmap_threshold;//!mmap分配阈值，mmap会动态调整此值，默认128K，32位max512KB，64位32MB
  INTERNAL_SIZE_T arena_test;    //!每个进程的分配区数量小于等于arena_test时，不会重用已有的分配区。64位为8，32位2
  INTERNAL_SIZE_T arena_max;     //!分配区的最大数量

  /* Memory map support */
  int n_mmaps;    //!当前堆用mmap分配的内存块的数量
  int n_mmaps_max;//!最大数量 默认65536，可变
  int max_n_mmaps;//!真正的最大数量
  /* the mmap_threshold is dynamic, until the user sets
     it manually, at which point we need to disable any
     dynamic behavior. */
  int no_dyn_threshold;//!默认0，表示开启mmap分配阈值动态调整

  /* Statistics */
  INTERNAL_SIZE_T mmapped_mem; //!mmap已分配内存的大小
  INTERNAL_SIZE_T max_mmapped_mem;//!mmap最大分配空间
  ...
  size_t tcache_bins;//! 链表数量
  size_t tcache_max_bytes;//!最大数量
  /* Maximum number of chunks in each bucket.  */
  size_t tcache_count;//! 每个链表最多节点数
  /* Maximum number of chunks to remove from the unsorted list, which
     aren't used to prefill the cache.  */
  size_t tcache_unsorted_limit;//! 可以从 unsorted bins中取出chunk的最大数量
#endif
};
...
munmap_chunk (mchunkptr p)
{
  ...
  assert (chunk_is_mmapped (p));//!断言：p是mmap分配的

  /* Do nothing if the chunk is a faked mmapped chunk in the dumped
     main arena.  We never free this memory.  */
  if (DUMPED_MAIN_ARENA_CHUNK (p))//!p在正常范围（堆空间起始~top chunk）否则退出
    return;

  uintptr_t block = (uintptr_t) p - prev_size (p);//!上一个堆的地址
  size_t total_size = prev_size (p) + size;//!两个堆的大小和
  /* Unfortunately we have to do the compilers job by hand here.  Normally
     we would test BLOCK and TOTAL-SIZE separately for compliance with the
     page size.  But gcc does not recognize the optimization possibility
     (in the moment at least) so we combine the two values into one before
     the bit test.  */
  if (__builtin_expect (((block | total_size) & (GLRO (dl_pagesize) - 1)) != 0, 0))//! (dl_pagesize)-1=0xfff 这里其实是在同时检查两个堆的地址是否是页对齐的
                                                                                   //! 因为 如果有一个页不对齐，total_size就会是不对齐的，而或上指针是为防止指针出问题而长度没问题
    {
      malloc_printerr (check_action, "munmap_chunk(): invalid pointer", //!错误打印函数 ，check_action=DEFAULT_CHECK_ACTION=3
                       chunk2mem (p), NULL);
      return;
    }

  atomic_decrement (&mp_.n_mmaps); //!记录mmap分配的内存块的数量减1
  atomic_add (&mp_.mmapped_mem, -total_size);//! 此指令等于 mp_.mmapped_mem+=-total_size

  /* If munmap failed the process virtual memory address space is in a
     bad shape.  Just leave the block hanging around, the process will
     terminate shortly anyway since not much can be done.  */
  __munmap ((char *) block, total_size);//!解mmap文件映射，linux系统调用，内容未知
                                        //! 为什么这里使用的是上一个chunk的地址和它们的长度和？解映射机制相关？
}
...
tcache_init(void)
{
  mstate ar_ptr;
  void *victim = 0; 
  const size_t bytes = sizeof (tcache_perthread_struct);//! tcache bins表 结构体

  if (tcache_shutting_down)//!tcache是否被关闭
    return;
...
#define MAYBE_INIT_TCACHE() \
  if (__glibc_unlikely (tcache == NULL)) \ //! tcache 不存在调用下面函数
    tcache_init();//!tcache初始化函数
...
__libc_malloc (size_t bytes)
{
  ...
  void *(*hook) (size_t, const void *)
    = atomic_forced_read (__malloc_hook);//!挂钩__malloc_hook
  if (__builtin_expect (hook != NULL, 0))
    return (*hook)(bytes, RETURN_ADDRESS (0));//!非空执行hook
#if USE_TCACHE
  /* int_free also calls request2size, be careful to not pad twice.  */
  size_t tbytes = request2size (bytes);//!添加chunk头区并向上对齐
  size_t tc_idx = csize2tidx (tbytes);//!将总空间转化为tcache偏移值，0x10一个单位

  MAYBE_INIT_TCACHE ();//!尝试初始化tcache

  DIAG_PUSH_NEEDS_COMMENT;//!用于记录当前的诊断状态
  if (tc_idx < mp_.tcache_bins//!偏移值在设置范围，未越界
  ...
  DIAG_POP_NEEDS_COMMENT;//!用于恢复诊断状态
#endif

  arena_get (ar_ptr, bytes);//!获取arena

  victim = _int_malloc (ar_ptr, bytes);//!核心函数
...
{
  mstate ar_ptr;       //! malloc 
  mchunkptr p;                          /* chunk corresponding to mem */

  void (*hook) (void *, const void *)
    = atomic_forced_read (__free_hook); //!hook挂钩
  if (__builtin_expect (hook != NULL, 0)) //!判断是否挂钩挂上
    {
      (*hook)(mem, RETURN_ADDRESS (0));//!挂上则执行函数
      return;
    }

  if (mem == 0)   //!指针不能空，虽然空了可以正常执行与返回         /* free(0) has no effect */
    return;

  p = mem2chunk (mem);//!偏移到chunk头 p=mem-2*size_sz

  if (chunk_is_mmapped (p)) //!判断是否为mmap分配的内存空间         /* release mmapped memory. */
    {
      /* See if the dynamic brk/mmap threshold needs adjusting.
	 Dumped fake mmapped chunks do not affect the threshold.  */
      if (!mp_.no_dyn_threshold//!是否开启mmap分配阈值动态调整
          && chunksize_nomask (p) > mp_.mmap_threshold //!chunk大小大于mmap分配阈值
          && chunksize_nomask (p) <= DEFAULT_MMAP_THRESHOLD_MAX//!小于最大分配阈值32MB
	  && !DUMPED_MAIN_ARENA_CHUNK (p))//!chunk指针是否在正常区间（堆空间起始~top chunk）
        {
          mp_.mmap_threshold = chunksize (p);//! mmap分配阈值设置为此chunk大小
          mp_.trim_threshold = 2 * mp_.mmap_threshold;//! top堆收缩阈值设置为新的mmap分配阈值的2倍
          LIBC_PROBE (memory_mallopt_free_dyn_thresholds, 2,//! https://xy2401.com/local-docs/gnu/manual.zh/libc/Memory-Allocation-Probes.html
                      mp_.mmap_threshold, mp_.trim_threshold);//!设置新的内存分配探针
        }
      munmap_chunk (p);//!解映射并调整相关数据
      return;
    }

  MAYBE_INIT_TCACHE ();//!为设置Tcache时初始化Tcache

  ar_ptr = arena_for_chunk (p);//!返回malloc_start地址 
...
_int_free (mstate av, mchunkptr p, int have_lock)
{
  ...
  if (__builtin_expect ((uintptr_t) p > (uintptr_t) -size, 0) //!p > -size防止后面p+size溢出
      || __builtin_expect (misaligned_chunk (p), 0))          //!misaligned_chunk判断指针是否符合字节对齐
    {
      errstr = "free(): invalid pointer";
    errout:
      if (!have_lock && locked) //!是否有锁，【locked作用未知】
        __libc_lock_unlock (av->mutex);//!解锁
      malloc_printerr (check_action, errstr, chunk2mem (p), av);//!内存错误打印并中断进程
      return;
    }
  /* We know that each chunk is at least MINSIZE bytes in size or a
     multiple of MALLOC_ALIGNMENT.  */
  if (__glibc_unlikely (size < MINSIZE || !aligned_OK (size)))//!大小是否合法
    {
      errstr = "free(): invalid size";
      goto errout;
    }

  check_inuse_chunk(av, p);//!非调试状态下无用

#if USE_TCACHE
  {
    size_t tc_idx = csize2tidx (size);//!将长度转化为序号 chunk size to tcache id number 正常情况最大为1032字节

    if (tcache //!是否启用
	&& tc_idx < mp_.tcache_bins //! 大小是否在tcache bins范围内
	&& tcache->counts[tc_idx] < mp_.tcache_count) //! bins是否满 
      {
	tcache_put (p, tc_idx);  //!  都满足放入tcahce bins，然后退出
	return;          
      }
  }
#endif
  ...
  */
  ...
  if ((unsigned long)(size) <= (unsigned long)(get_max_fast ())//!判断是否符合fastbin大小
	...
	bordering top into fastbins
      */
      && (chunk_at_offset(p, size) != av->top)//!下一个堆非top堆
#endif
      ) {

    if (__builtin_expect (chunksize_nomask (chunk_at_offset (p, size))
			  <= 2 * SIZE_SZ, 0)//!下一个堆的大小是否<=2*SIZE
	|| __builtin_expect (chunksize (chunk_at_offset (p, size))
			     >= av->system_mem, 0))//!下一个堆的大小是否>系统分配大小   “无效长度”
      {
	/* We might not have a lock at this point and concurrent modifications
	   of system_mem might have let to a false positive.  Redo the test
	   after getting the lock.  */
	if (have_lock  //!有锁
	    || ({ assert (locked == 0);//! 【locked未知】
		  __libc_lock_lock (av->mutex);//! 【上锁？】
		  locked = 1;
		  chunksize_nomask (chunk_at_offset (p, size)) <= 2 * SIZE_SZ //! 下一个堆的大小小于等于0x10
		    || chunksize (chunk_at_offset (p, size)) >= av->system_mem;//! 或者大于系统最大允许空间
	      }))
	  {
    ...
    free_perturb (chunk2mem(p), size - 2 * SIZE_SZ);//!usedata处理多半不处理

    set_fastchunks(av);//!设置malloc_state为存在fastbin
    unsigned int idx = fastbin_index(size);//!所属计算
    fb = &fastbin (av, idx);//!拉取地址
	...
	/* Check that the top of the bin is not the record we are going to add
	   (i.e., double free).  */
	if (__builtin_expect (old == p, 0))//!防这一次释放上一次的
	  {
  ...
  */
    ...
    nextchunk = chunk_at_offset(p, size); //!保存向增相邻chunk地址

    /* Lightweight tests: check whether the block is already the
       top block.  */
    if (__glibc_unlikely (p == av->top))//!检查是否是top chunk
      {
	errstr = "double free or corruption (top)";
	goto errout;
      }
    /* Or whether the next chunk is beyond the boundaries of the arena.  */
    if (__builtin_expect (contiguous (av)//!检查下一个堆是否超过堆边界
			  && (char *) nextchunk
    ...
    if (__glibc_unlikely (!prev_inuse(nextchunk)))//!检查p堆是否被使用
      {
	errstr = "double free or corruption (!prev)";
	goto errout;
      }

    nextsize = chunksize(nextchunk);//!下个堆的长度
    if (__builtin_expect (chunksize_nomask (nextchunk) <= 2 * SIZE_SZ, 0)
	|| __builtin_expect (nextsize >= av->system_mem, 0))//!长度是否合法
      {
	errstr = "free(): invalid next size (normal)";
	goto errout;
      }

    free_perturb (chunk2mem(p), size - 2 * SIZE_SZ);//!数据清除？（某种情况下，但不会是清零）

    /* consolidate backward */
    if (!prev_inuse(p)) {//!上一个chunk未被使用时
      prevsize = prev_size (p);//!获取上一个堆长度
      size += prevsize; //!长度合并
      p = chunk_at_offset(p, -((long) prevsize));//!指针迁移，旧堆数据实存名亡，但新的chunk的size未修改
      unlink(av, p, bck, fwd);//!解链p的上一个chunk（释放状态）
    }

    if (nextchunk != av->top) {//!判断是否为top chunk
      /* get and clear inuse bit */
      nextinuse = inuse_bit_at_offset(nextchunk, nextsize);//!读取p的下一个chunk的使用状态 

      /* consolidate forward */
      if (!nextinuse) {//!未被使用
	unlink(av, nextchunk, bck, fwd);//!p的nextchunk解链
	size += nextsize;
      } else
	clear_inuse_bit_at_offset(nextchunk, 0);//!标记p为未使用
	...
	been given one chance to be used in malloc.
      */
      ...
      bck = unsorted_chunks(av);//!拉起unsorted_chunk中第一chunk
      fwd = bck->fd;//!如上
      if (__glibc_unlikely (fwd->bk != bck))
	{
      ...
      p->fd = fwd;//!将经过合并判定的p链接入unsorted
      p->bk = bck;//?开始修改新p
      if (!in_smallbin_range(size))//!大小判断
	{
	  p->fd_nextsize = NULL;
	  p->bk_nextsize = NULL;
	}
      ...
      set_head(p, size | PREV_INUSE); //!修改新p大小和位，标记上面一个堆为启用（继承）
      set_foot(p, size);//!下个新p的下一个相邻chunk的prev_size值

      check_free_chunk(av, p);//?未知定义
    }
    ...
    */
      ...
      size += nextsize;
      set_head(p, size | PREV_INUSE);//!标记
      av->top = p;//!修改top指针
      check_chunk(av, p);//?未知定义
    }
    ...
    */
    ...
    if ((unsigned long)(size) >= FASTBIN_CONSOLIDATION_THRESHOLD) {//!判断size是否达到fastbin整合门槛
      if (have_fastchunks(av))//!fastbin中存在元素
	malloc_consolidate(av);//? 未细看 合并fastbin中所有的可合并chunk

      if (av == &main_arena) {//!是否为主线程
...
malloc_printerr (int action, const char *str, void *ptr, mstate ar_ptr)
{
  ...
  if (ar_ptr)
    set_arena_corrupt (ar_ptr);//!设置malloc_state结构体中flag的ARENA_CORRUPTION_BIT标志位为1。`应该标志着内存出现损坏而进行锁定`

  if ((action & 5) == 5)//! 行为指导参数？
